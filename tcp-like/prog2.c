...
pkt_t make_pkt(int seqnum, int acknum, msg_t message);
char packet_is_corrupt(pkt_t packet);
...
void tolayer5(int AorB, msg_t sent_msg);

/* Minhas variaveis globais */
#define N 8
#define TIMEOUT_TIME 20.0
int base = 1;
int nextseqnum = 1;
pkt_t sndpkt[N];
...
int send_queue_start = 0;
int send_queue_end = 1; 
msg_t send_queue[50];
...
pkt_t make_pkt(int seqnum, int acknum, msg_t message)
{
  ...
  for (i=0; i<20; i++)
  {
    packet.checksum += packet.payload.data[i];
  }
...
char packet_is_corrupt(pkt_t packet)
{
  ...
  verify_checksum += packet.seqnum;
  verify_checksum += packet.acknum;
  for (i=0; i < 20; i++)
  {
    verify_checksum += packet.payload.data[i];
  }
  return verify_checksum != packet.checksum;
}

/* As funcoes que eu tenho que preencher */
...
void A_input(pkt_t packet)
{
  if (!packet_is_corrupt(packet))
  {
    ...
    {
      printf("ACK %d repetido recebido\n", packet.acknum);
    }
  }
  else
  {
    printf("ACK %d corrompido\n", packet.acknum);
  }
   ...
   while (1) {
	  ...
	  break;                        /* all done with simulation */
        if (eventptr->evtype == FROM_LAYER5 ) {
            ...
            for (i=0; i<20; i++)  
                pkt2give.payload.data[i] = eventptr->pktptr->payload.data[i];
...
/* to do something with the packet after we return back to him/her */ 
 ...
 for (i=0; i<20; i++)
    mypktptr->payload.data[i] = packet.payload.data[i];
 if (TRACE>2)  {
   printf("          TOLAYER3: seq: %d, ack %d, check: %d ", mypktptr->seqnum,
	  mypktptr->acknum,  mypktptr->checksum);
    for (i=0; i<20; i++)
        printf("%c",mypktptr->payload.data[i]);
    printf("\n");
   }
...
/* for (q=evlist; q!=NULL && q->next!=NULL; q = q->next) */
 ...
 if (jimsrand() < corruptprob)  {
    ncorrupt++;
    if ( (x = jimsrand()) < .75)
       mypktptr->payload.data[0]='Z';   /* corrupt payload */
      else if (x < .875)
